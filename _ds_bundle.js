// TEDxAmsterdam Design System — browser bundle
// Components: Accordion, ArrowLink, Badge, Breadcrumb, Button, Checkbox, CheckboxGroup, FormField, Input, Logo, MediaCard, Modal, NavigationBar, Radio, RadioGroup, SectionHeader, Select, SocialLink, Stat, Tag, Tabs, Textarea, Toast
// Generated by scripts/build-bundle.js — do not edit manually.
(function () {
  'use strict';
  var __ds_ns = window.TEDxAmsterdamDesignSystem = window.TEDxAmsterdamDesignSystem || {};
  __ds_ns.__errors = [];

// components/core/Accordion.tsx
const React = window.React;
// components/utils/cn.ts
function cn(...classes) {
  return classes.filter(Boolean).join(" ");
}

// components/core/Accordion.module.css
var Accordion_default = {
  root: "Accordion_root",
  item: "Accordion_item",
  btn: "Accordion_btn",
  question: "Accordion_question",
  open: "Accordion_open",
  icon: "Accordion_icon",
  iconH: "Accordion_iconH",
  iconV: "Accordion_iconV",
  panel: "Accordion_panel",
  answer: "Accordion_answer"
};

// components/core/Accordion.tsx
function Accordion({ items = [], defaultOpen = 0, className, style, ...rest }) {
  const [open, setOpen] = React.useState(defaultOpen);
  const uid = React.useId();
  return /* @__PURE__ */ React.createElement("div", { className, style, ...rest }, items.map((it, i) => {
    const isOpen = open === i;
    const btnId = `accordion-${uid}-btn-${i}`;
    const panelId = `accordion-${uid}-panel-${i}`;
    return /* @__PURE__ */ React.createElement("div", { key: i, className: Accordion_default.item }, /* @__PURE__ */ React.createElement(
      "button",
      {
        id: btnId,
        "aria-expanded": isOpen,
        "aria-controls": panelId,
        onClick: () => setOpen(isOpen ? -1 : i),
        className: Accordion_default.btn
      },
      /* @__PURE__ */ React.createElement("span", { className: cn(Accordion_default.question, isOpen && Accordion_default.open) }, it.q),
      /* @__PURE__ */ React.createElement("span", { "aria-hidden": "true", className: cn(Accordion_default.icon, isOpen && Accordion_default.open) }, /* @__PURE__ */ React.createElement("span", { className: Accordion_default.iconH }), /* @__PURE__ */ React.createElement("span", { className: cn(Accordion_default.iconV, isOpen && Accordion_default.open) }))
    ), /* @__PURE__ */ React.createElement(
      "div",
      {
        id: panelId,
        role: "region",
        "aria-labelledby": btnId,
        "aria-hidden": !isOpen,
        className: cn(Accordion_default.panel, isOpen && Accordion_default.open)
      },
      /* @__PURE__ */ React.createElement("p", { className: Accordion_default.answer }, it.a)
    ));
  }));
}

// components/core/ArrowLink.tsx
const React2 = window.React;
// components/core/ArrowLink.module.css
var ArrowLink_default = {
  link: "ArrowLink_link",
  red: "ArrowLink_red",
  white: "ArrowLink_white"
};

// components/core/ArrowLink.tsx
function ArrowLink({ children, href = "#", color = "red", style, className, ...rest }) {
  return /* @__PURE__ */ React2.createElement(
    "a",
    {
      href,
      className: cn(ArrowLink_default.link, ArrowLink_default[color], className),
      style,
      ...rest
    },
    children,
    /* @__PURE__ */ React2.createElement("svg", { width: "18", height: "9", viewBox: "0 0 18 9", fill: "none", "aria-hidden": "true" }, /* @__PURE__ */ React2.createElement("path", { d: "M0 4.5h15", stroke: "currentColor", strokeWidth: "1.6" }), /* @__PURE__ */ React2.createElement("path", { d: "M11.5 1l4 3.5-4 3.5", stroke: "currentColor", strokeWidth: "1.6", fill: "none" }))
  );
}

// components/core/Badge.tsx
const React3 = window.React;
// components/core/Badge.module.css
var Badge_default = {
  badge: "Badge_badge",
  sm: "Badge_sm",
  md: "Badge_md",
  red: "Badge_red",
  white: "Badge_white",
  muted: "Badge_muted"
};

// components/core/Badge.tsx
function Badge({ label, color = "red", size = "md", className, style, ...rest }) {
  return /* @__PURE__ */ React3.createElement("span", { className: cn(Badge_default.badge, Badge_default[color], Badge_default[size], className), style, ...rest }, label);
}

// components/core/Breadcrumb.tsx
const React4 = window.React;
// components/core/Breadcrumb.module.css
var Breadcrumb_default = {
  list: "Breadcrumb_list",
  item: "Breadcrumb_item",
  link: "Breadcrumb_link",
  current: "Breadcrumb_current",
  separator: "Breadcrumb_separator"
};

// components/core/Breadcrumb.tsx
function Breadcrumb({ items = [], ariaLabel = "Breadcrumb", className, style, ...rest }) {
  return /* @__PURE__ */ React4.createElement("nav", { ...rest, "aria-label": ariaLabel, className, style }, /* @__PURE__ */ React4.createElement("ol", { className: Breadcrumb_default.list }, items.map((item, i) => {
    const isCurrent = i === items.length - 1;
    return /* @__PURE__ */ React4.createElement("li", { key: item.href ?? item.label, className: Breadcrumb_default.item }, !isCurrent && item.href ? /* @__PURE__ */ React4.createElement("a", { href: item.href, className: Breadcrumb_default.link }, item.label) : /* @__PURE__ */ React4.createElement(
      "span",
      {
        "aria-current": isCurrent ? "page" : void 0,
        className: isCurrent ? Breadcrumb_default.current : Breadcrumb_default.link
      },
      item.label
    ), !isCurrent && /* @__PURE__ */ React4.createElement("span", { "aria-hidden": "true", className: Breadcrumb_default.separator }, "\u203A"));
  })));
}

// components/core/Button.tsx
const React5 = window.React;
// components/core/Button.module.css
var Button_default = {
  btn: "Button_btn",
  disabled: "Button_disabled",
  sm: "Button_sm",
  md: "Button_md",
  lg: "Button_lg",
  primary: "Button_primary",
  secondary: "Button_secondary",
  ghost: "Button_ghost",
  arrow: "Button_arrow"
};

// components/core/Button.tsx
function Button({
  variant = "primary",
  size = "md",
  disabled = false,
  arrow = false,
  className,
  style,
  children,
  ...rest
}) {
  const classes = cn(Button_default.btn, Button_default[variant], Button_default[size], disabled && Button_default.disabled, className);
  const handleClick = (e) => {
    if (disabled) {
      e.preventDefault();
      e.stopPropagation();
      return;
    }
    rest.onClick?.(e);
  };
  const content = /* @__PURE__ */ React5.createElement(React5.Fragment, null, children, arrow && /* @__PURE__ */ React5.createElement("span", { className: Button_default.arrow, "aria-hidden": "true" }, "\u2192"));
  if (rest.href !== void 0) {
    const { href, ...anchorRest } = rest;
    return /* @__PURE__ */ React5.createElement(
      "a",
      {
        ...anchorRest,
        href: disabled ? void 0 : href,
        "aria-disabled": disabled || void 0,
        className: classes,
        style,
        onClick: handleClick
      },
      content
    );
  }
  const buttonRest = rest;
  return /* @__PURE__ */ React5.createElement(
    "button",
    {
      ...buttonRest,
      type: buttonRest.type ?? "button",
      "aria-disabled": disabled || void 0,
      className: classes,
      style,
      onClick: handleClick
    },
    content
  );
}

// components/core/Checkbox.tsx
const React6 = window.React;
// components/core/Checkbox.module.css
var Checkbox_default = {
  label: "Checkbox_label",
  disabled: "Checkbox_disabled",
  indicator: "Checkbox_indicator",
  checked: "Checkbox_checked",
  tick: "Checkbox_tick",
  nativeInput: "Checkbox_nativeInput",
  text: "Checkbox_text"
};

// components/core/Checkbox.tsx
function Checkbox({ label, checked, onChange, disabled = false, id, className, style, ...rest }) {
  const checkId = id || `checkbox-${String(label).toLowerCase().replace(/\s+/g, "-")}`;
  return /* @__PURE__ */ React6.createElement(
    "label",
    {
      ...rest,
      htmlFor: checkId,
      className: cn(Checkbox_default.label, disabled && Checkbox_default.disabled, className),
      style
    },
    /* @__PURE__ */ React6.createElement("span", { className: cn(Checkbox_default.indicator, checked && Checkbox_default.checked) }, checked && /* @__PURE__ */ React6.createElement(
      "svg",
      {
        "aria-hidden": "true",
        width: "10",
        height: "8",
        viewBox: "0 0 10 8",
        fill: "none",
        className: Checkbox_default.tick
      },
      /* @__PURE__ */ React6.createElement(
        "path",
        {
          d: "M1 4l2.5 2.5L9 1",
          stroke: "white",
          strokeWidth: "1.8",
          strokeLinecap: "round",
          strokeLinejoin: "round"
        }
      )
    ), /* @__PURE__ */ React6.createElement(
      "input",
      {
        id: checkId,
        type: "checkbox",
        checked,
        onChange: (e) => onChange?.(e.target.checked),
        disabled,
        className: Checkbox_default.nativeInput
      }
    )),
    /* @__PURE__ */ React6.createElement("span", { className: Checkbox_default.text }, label)
  );
}

// components/core/CheckboxGroup.tsx
const React7 = window.React;
// components/core/CheckboxGroup.module.css
var CheckboxGroup_default = {
  fieldset: "CheckboxGroup_fieldset",
  legend: "CheckboxGroup_legend"
};

// components/core/CheckboxGroup.tsx
function CheckboxGroup({ legend, children, className, style, ...rest }) {
  return /* @__PURE__ */ React7.createElement("fieldset", { ...rest, className: cn(CheckboxGroup_default.fieldset, className), style }, legend && /* @__PURE__ */ React7.createElement("legend", { className: CheckboxGroup_default.legend }, legend), children);
}

// components/core/FormField.tsx
const React8 = window.React;
// components/core/FormField.module.css
var FormField_default = {
  wrapper: "FormField_wrapper",
  label: "FormField_label",
  required: "FormField_required",
  hint: "FormField_hint",
  error: "FormField_error"
};

// components/core/FormField.tsx
var FormFieldContext = React8.createContext(null);
function useFormField() {
  return React8.useContext(FormFieldContext);
}
function FormField({ label, htmlFor, hint, error, required = false, children, className, style, ...rest }) {
  const autoId = React8.useId();
  const fieldId = htmlFor ?? `field-${autoId}`;
  const errorId = `${fieldId}-formfield-error`;
  const ctx = React8.useMemo(
    () => ({ id: fieldId, errorId, invalid: Boolean(error), required }),
    [fieldId, errorId, error, required]
  );
  return /* @__PURE__ */ React8.createElement(FormFieldContext.Provider, { value: ctx }, /* @__PURE__ */ React8.createElement("div", { ...rest, className: cn(FormField_default.wrapper, className), style }, label && /* @__PURE__ */ React8.createElement("label", { htmlFor: fieldId, className: FormField_default.label }, label, required && /* @__PURE__ */ React8.createElement("span", { "aria-hidden": "true", className: FormField_default.required }, "*")), hint && /* @__PURE__ */ React8.createElement("span", { className: FormField_default.hint }, hint), children, /* @__PURE__ */ React8.createElement(
    "span",
    {
      id: errorId,
      "aria-live": "polite",
      className: FormField_default.error
    },
    error ?? ""
  )));
}

// components/core/Input.tsx
const React9 = window.React;
// components/core/Input.module.css
var Input_default = {
  wrapper: "Input_wrapper",
  dark: "Input_dark",
  label: "Input_label",
  light: "Input_light",
  input: "Input_input",
  error: "Input_error",
  errorMsg: "Input_errorMsg"
};

// components/core/Input.tsx
function Input({
  label = "Email",
  type = "email",
  placeholder = "",
  value,
  onChange,
  tone = "dark",
  error,
  id,
  className,
  style,
  ...rest
}) {
  const field = useFormField();
  const inputId = id || field?.id || `input-${String(label).toLowerCase().replace(/\s+/g, "-")}`;
  const errorId = `${inputId}-error`;
  const describedBy = [errorId, field?.errorId].filter(Boolean).join(" ");
  return /* @__PURE__ */ React9.createElement("label", { className: cn(Input_default.wrapper, Input_default[tone], className), style }, /* @__PURE__ */ React9.createElement("span", { className: Input_default.label }, label), /* @__PURE__ */ React9.createElement(
    "input",
    {
      id: inputId,
      type,
      placeholder,
      value,
      onChange,
      required: field?.required || void 0,
      "aria-invalid": Boolean(error) || Boolean(field?.invalid),
      "aria-describedby": describedBy,
      className: cn(Input_default.input, error && Input_default.error),
      ...rest
    }
  ), /* @__PURE__ */ React9.createElement("span", { id: errorId, className: Input_default.errorMsg, "aria-live": "polite" }, error ?? ""));
}

// components/core/Logo.tsx
const React10 = window.React;
// components/core/Logo.module.css
var Logo_default = {
  wrap: "Logo_wrap",
  img: "Logo_img"
};

// components/core/Logo.tsx
function Logo({ tone = "light-on-dark", width = 230, base = "", href, className, style, ...rest }) {
  const src = tone === "dark-on-light" ? base + "assets/logo-tedxamsterdam.svg" : base + "assets/logo-tedxamsterdam-white.png";
  const img = /* @__PURE__ */ React10.createElement(
    "img",
    {
      src,
      alt: "TEDxAmsterdam",
      className: Logo_default.img,
      style: { width }
    }
  );
  return href ? /* @__PURE__ */ React10.createElement("a", { href, className: cn(Logo_default.wrap, className), style, ...rest }, img) : /* @__PURE__ */ React10.createElement("span", { className: cn(Logo_default.wrap, className), style, ...rest }, img);
}

// components/core/MediaCard.tsx
const React11 = window.React;
// components/core/MediaCard.module.css
var MediaCard_default = {
  card: "MediaCard_card",
  imgOuter: "MediaCard_imgOuter",
  imgInner: "MediaCard_imgInner",
  category: "MediaCard_category",
  title: "MediaCard_title",
  description: "MediaCard_description",
  date: "MediaCard_date"
};

// components/core/MediaCard.tsx
function MediaCard({
  image,
  category,
  title,
  description,
  date,
  href = "#",
  ratio = "4 / 3",
  className,
  style,
  ...rest
}) {
  return /* @__PURE__ */ React11.createElement(
    "a",
    {
      href,
      className: cn(MediaCard_default.card, className),
      style,
      ...rest
    },
    /* @__PURE__ */ React11.createElement(
      "div",
      {
        className: MediaCard_default.imgOuter,
        style: { aspectRatio: ratio }
      },
      /* @__PURE__ */ React11.createElement(
        "div",
        {
          className: MediaCard_default.imgInner,
          style: {
            background: image ? `url(${image}) center / cover no-repeat` : "var(--grad-tile)"
          }
        }
      ),
      category && /* @__PURE__ */ React11.createElement("span", { className: MediaCard_default.category }, category)
    ),
    /* @__PURE__ */ React11.createElement("h3", { className: MediaCard_default.title }, title),
    description && /* @__PURE__ */ React11.createElement("p", { className: MediaCard_default.description }, description),
    date && /* @__PURE__ */ React11.createElement("span", { className: MediaCard_default.date }, date)
  );
}

// components/core/Modal.tsx
const React12 = window.React;
const { createPortal } = window.ReactDOM;
// components/core/Modal.module.css
var Modal_default = {
  overlay: "Modal_overlay",
  dialog: "Modal_dialog",
  header: "Modal_header",
  title: "Modal_title",
  closeBtn: "Modal_closeBtn",
  body: "Modal_body"
};

// components/core/Modal.tsx
var maxWidths = {
  sm: 480,
  md: 640,
  lg: 800
};
var FOCUSABLE = 'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])';
function Modal({ isOpen, onClose, title, children, size = "md", hideCloseButton = false, className, style, ...rest }) {
  const dialogRef = React12.useRef(null);
  const restoreFocusRef = React12.useRef(null);
  const titleId = `modal-title-${React12.useId()}`;
  React12.useEffect(() => {
    if (!isOpen) return;
    const prevOverflow = document.body.style.overflow;
    const prevPadding = document.body.style.paddingRight;
    const scrollbar = window.innerWidth - document.documentElement.clientWidth;
    document.body.style.overflow = "hidden";
    if (scrollbar > 0) document.body.style.paddingRight = `${scrollbar}px`;
    const onKey = (e) => {
      if (e.key === "Escape") onClose();
    };
    document.addEventListener("keydown", onKey);
    return () => {
      document.body.style.overflow = prevOverflow;
      document.body.style.paddingRight = prevPadding;
      document.removeEventListener("keydown", onKey);
    };
  }, [isOpen, onClose]);
  React12.useEffect(() => {
    if (!isOpen || !dialogRef.current) return;
    restoreFocusRef.current = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    dialogRef.current.querySelectorAll(FOCUSABLE)[0]?.focus();
    const trap = (e) => {
      if (e.key !== "Tab" || !dialogRef.current) return;
      const focusable = Array.from(dialogRef.current.querySelectorAll(FOCUSABLE));
      const first = focusable[0];
      const last = focusable[focusable.length - 1];
      if (e.shiftKey) {
        if (document.activeElement === first) {
          e.preventDefault();
          last?.focus();
        }
      } else {
        if (document.activeElement === last) {
          e.preventDefault();
          first?.focus();
        }
      }
    };
    document.addEventListener("keydown", trap);
    return () => {
      document.removeEventListener("keydown", trap);
      restoreFocusRef.current?.focus();
    };
  }, [isOpen]);
  if (!isOpen) return null;
  return createPortal(
    /* @__PURE__ */ React12.createElement("div", { className: Modal_default.overlay, onClick: onClose }, /* @__PURE__ */ React12.createElement(
      "div",
      {
        ...rest,
        ref: dialogRef,
        role: "dialog",
        "aria-modal": "true",
        "aria-labelledby": title ? titleId : void 0,
        onClick: (e) => e.stopPropagation(),
        className: cn(Modal_default.dialog, className),
        style: { maxWidth: maxWidths[size], ...style }
      },
      /* @__PURE__ */ React12.createElement("div", { className: Modal_default.header }, title && /* @__PURE__ */ React12.createElement("h2", { id: titleId, className: Modal_default.title }, title), !hideCloseButton && /* @__PURE__ */ React12.createElement(
        "button",
        {
          onClick: onClose,
          "aria-label": "Close dialog",
          className: Modal_default.closeBtn
        },
        "\xD7"
      )),
      /* @__PURE__ */ React12.createElement("div", { className: Modal_default.body }, children)
    )),
    document.body
  );
}

// components/core/NavigationBar.tsx
const React13 = window.React;
// components/core/NavigationBar.module.css
var NavigationBar_default = {
  header: "NavigationBar_header",
  skipLink: "NavigationBar_skipLink",
  bar: "NavigationBar_bar",
  logoLink: "NavigationBar_logoLink",
  logoImg: "NavigationBar_logoImg",
  desktopNav: "NavigationBar_desktopNav",
  navLinks: "NavigationBar_navLinks",
  navLink: "NavigationBar_navLink",
  active: "NavigationBar_active",
  ctaLink: "NavigationBar_ctaLink",
  hamburger: "NavigationBar_hamburger",
  mobileMenu: "NavigationBar_mobileMenu",
  mobileLink: "NavigationBar_mobileLink",
  mobileCta: "NavigationBar_mobileCta"
};

// components/core/NavigationBar.tsx
function NavigationBar({
  links = [],
  ctaLabel,
  ctaHref,
  logoHref = "/",
  activePath,
  base = "",
  skipTarget = "#main-content",
  className,
  style,
  ...rest
}) {
  const [menuOpen, setMenuOpen] = React13.useState(false);
  const menuId = `nav-mobile-menu-${React13.useId()}`;
  const hamburgerRef = React13.useRef(null);
  React13.useEffect(() => {
    if (!menuOpen) return;
    const onKey = (e) => {
      if (e.key === "Escape") {
        setMenuOpen(false);
        hamburgerRef.current?.focus();
      }
    };
    document.addEventListener("keydown", onKey);
    return () => document.removeEventListener("keydown", onKey);
  }, [menuOpen]);
  const logoSrc = base + "assets/logo-tedxamsterdam-white.png";
  return /* @__PURE__ */ React13.createElement("header", { ...rest, className: cn(NavigationBar_default.header, className), style }, /* @__PURE__ */ React13.createElement("a", { href: skipTarget, className: NavigationBar_default.skipLink }, "Skip to main content"), /* @__PURE__ */ React13.createElement("div", { className: NavigationBar_default.bar }, /* @__PURE__ */ React13.createElement("a", { href: logoHref, className: NavigationBar_default.logoLink }, /* @__PURE__ */ React13.createElement("img", { src: logoSrc, alt: "TEDxAmsterdam", className: NavigationBar_default.logoImg })), /* @__PURE__ */ React13.createElement("div", { className: NavigationBar_default.desktopNav }, /* @__PURE__ */ React13.createElement("nav", { "aria-label": "Main navigation", className: NavigationBar_default.navLinks }, links.map((link) => /* @__PURE__ */ React13.createElement(
    "a",
    {
      key: link.href,
      href: link.href,
      "aria-current": activePath === link.href ? "page" : void 0,
      className: cn(NavigationBar_default.navLink, activePath === link.href && NavigationBar_default.active)
    },
    link.label
  ))), ctaHref && ctaLabel && /* @__PURE__ */ React13.createElement("a", { href: ctaHref, className: NavigationBar_default.ctaLink }, ctaLabel)), /* @__PURE__ */ React13.createElement(
    "button",
    {
      ref: hamburgerRef,
      type: "button",
      className: NavigationBar_default.hamburger,
      "aria-expanded": menuOpen,
      "aria-controls": menuId,
      "aria-label": menuOpen ? "Close menu" : "Open menu",
      onClick: () => setMenuOpen(!menuOpen)
    },
    /* @__PURE__ */ React13.createElement("svg", { "aria-hidden": "true", width: "24", height: "18", viewBox: "0 0 24 18", fill: "none" }, menuOpen ? /* @__PURE__ */ React13.createElement(React13.Fragment, null, /* @__PURE__ */ React13.createElement("line", { x1: "2", y1: "2", x2: "22", y2: "16", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" }), /* @__PURE__ */ React13.createElement("line", { x1: "22", y1: "2", x2: "2", y2: "16", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" })) : /* @__PURE__ */ React13.createElement(React13.Fragment, null, /* @__PURE__ */ React13.createElement("line", { x1: "0", y1: "2", x2: "24", y2: "2", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" }), /* @__PURE__ */ React13.createElement("line", { x1: "0", y1: "9", x2: "24", y2: "9", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" }), /* @__PURE__ */ React13.createElement("line", { x1: "0", y1: "16", x2: "24", y2: "16", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" })))
  )), menuOpen && /* @__PURE__ */ React13.createElement("div", { id: menuId, className: NavigationBar_default.mobileMenu }, /* @__PURE__ */ React13.createElement("nav", { "aria-label": "Mobile navigation" }, links.map((link) => /* @__PURE__ */ React13.createElement(
    "a",
    {
      key: link.href,
      href: link.href,
      onClick: () => setMenuOpen(false),
      "aria-current": activePath === link.href ? "page" : void 0,
      className: cn(NavigationBar_default.navLink, NavigationBar_default.mobileLink, activePath === link.href && NavigationBar_default.active)
    },
    link.label
  ))), ctaHref && ctaLabel && /* @__PURE__ */ React13.createElement(
    "a",
    {
      href: ctaHref,
      onClick: () => setMenuOpen(false),
      className: cn(NavigationBar_default.ctaLink, NavigationBar_default.mobileCta)
    },
    ctaLabel
  )));
}

// components/core/Radio.tsx
const React14 = window.React;
// components/core/Radio.module.css
var Radio_default = {
  fieldset: "Radio_fieldset",
  legend: "Radio_legend",
  optionLabel: "Radio_optionLabel",
  disabled: "Radio_disabled",
  indicator: "Radio_indicator",
  selected: "Radio_selected",
  dot: "Radio_dot",
  nativeInput: "Radio_nativeInput",
  text: "Radio_text"
};

// components/core/Radio.tsx
function Radio({ legend, options = [], value, onChange, name, disabled = false, className, style, ...rest }) {
  return /* @__PURE__ */ React14.createElement("fieldset", { ...rest, className: cn(Radio_default.fieldset, className), style }, legend && /* @__PURE__ */ React14.createElement("legend", { className: Radio_default.legend }, legend), options.map((opt) => {
    const isSelected = value === opt.value;
    const radioId = `${name}-${opt.value}`;
    return /* @__PURE__ */ React14.createElement(
      "label",
      {
        key: opt.value,
        htmlFor: radioId,
        className: cn(Radio_default.optionLabel, disabled && Radio_default.disabled)
      },
      /* @__PURE__ */ React14.createElement("span", { className: cn(Radio_default.indicator, isSelected && Radio_default.selected) }, isSelected && /* @__PURE__ */ React14.createElement("span", { className: Radio_default.dot }), /* @__PURE__ */ React14.createElement(
        "input",
        {
          id: radioId,
          type: "radio",
          name,
          value: opt.value,
          checked: isSelected,
          onChange: () => onChange?.(opt.value),
          disabled,
          className: Radio_default.nativeInput
        }
      )),
      /* @__PURE__ */ React14.createElement("span", { className: Radio_default.text }, opt.label)
    );
  }));
}

// components/core/RadioGroup.tsx
const React15 = window.React;
// components/core/RadioGroup.module.css
var RadioGroup_default = {
  fieldset: "RadioGroup_fieldset",
  legend: "RadioGroup_legend",
  optionLabel: "RadioGroup_optionLabel",
  disabled: "RadioGroup_disabled",
  indicator: "RadioGroup_indicator",
  selected: "RadioGroup_selected",
  dot: "RadioGroup_dot",
  nativeInput: "RadioGroup_nativeInput",
  text: "RadioGroup_text"
};

// components/core/RadioGroup.tsx
function RadioOption({ value, label, _name, _groupValue, _onChange, _disabled }) {
  const id = `${_name}-${value}`;
  const isSelected = _groupValue === value;
  return /* @__PURE__ */ React15.createElement(
    "label",
    {
      htmlFor: id,
      className: cn(RadioGroup_default.optionLabel, _disabled && RadioGroup_default.disabled)
    },
    /* @__PURE__ */ React15.createElement("span", { className: cn(RadioGroup_default.indicator, isSelected && RadioGroup_default.selected) }, isSelected && /* @__PURE__ */ React15.createElement("span", { className: RadioGroup_default.dot }), /* @__PURE__ */ React15.createElement(
      "input",
      {
        id,
        type: "radio",
        name: _name,
        value,
        checked: isSelected,
        onChange: () => _onChange?.(value),
        disabled: _disabled,
        className: RadioGroup_default.nativeInput
      }
    )),
    /* @__PURE__ */ React15.createElement("span", { className: RadioGroup_default.text }, label)
  );
}
function RadioGroupRoot({ name, legend, value, onChange, disabled = false, children, className, style, ...rest }) {
  return /* @__PURE__ */ React15.createElement("fieldset", { ...rest, className: cn(RadioGroup_default.fieldset, className), style }, legend && /* @__PURE__ */ React15.createElement("legend", { className: RadioGroup_default.legend }, legend), React15.Children.map(
    children,
    (child) => React15.isValidElement(child) ? React15.cloneElement(child, {
      _name: name,
      _groupValue: value,
      _onChange: onChange,
      _disabled: disabled
    }) : child
  ));
}
var RadioGroup = RadioGroupRoot;
RadioGroup.Option = RadioOption;

// components/core/SectionHeader.tsx
const React17 = window.React;
// components/core/Tag.tsx
const React16 = window.React;
// components/core/Tag.module.css
var Tag_default = {
  tag: "Tag_tag",
  red: "Tag_red",
  white: "Tag_white",
  muted: "Tag_muted",
  dot: "Tag_dot"
};

// components/core/Tag.tsx
function Tag({ children, color = "red", dot = false, style, className, ...rest }) {
  return /* @__PURE__ */ React16.createElement(
    "span",
    {
      className: cn(Tag_default.tag, Tag_default[color], className),
      style,
      ...rest
    },
    dot && /* @__PURE__ */ React16.createElement(
      "span",
      {
        "aria-hidden": "true",
        className: Tag_default.dot
      }
    ),
    children
  );
}

// components/core/SectionHeader.module.css
var SectionHeader_default = {
  root: "SectionHeader_root",
  left: "SectionHeader_left",
  center: "SectionHeader_center",
  title: "SectionHeader_title",
  description: "SectionHeader_description",
  linkWrap: "SectionHeader_linkWrap"
};

// components/core/SectionHeader.tsx
function SectionHeader({
  eyebrow,
  title,
  description,
  link,
  linkHref = "#",
  align = "left",
  className,
  style,
  ...rest
}) {
  return /* @__PURE__ */ React17.createElement(
    "div",
    {
      className: cn(SectionHeader_default.root, align === "center" ? SectionHeader_default.center : SectionHeader_default.left, className),
      style,
      ...rest
    },
    eyebrow && /* @__PURE__ */ React17.createElement(Tag, null, eyebrow),
    /* @__PURE__ */ React17.createElement("h2", { className: SectionHeader_default.title }, title),
    description && /* @__PURE__ */ React17.createElement("p", { className: SectionHeader_default.description }, description),
    link && /* @__PURE__ */ React17.createElement("div", { className: SectionHeader_default.linkWrap }, /* @__PURE__ */ React17.createElement(ArrowLink, { href: linkHref }, link))
  );
}

// components/core/Select.tsx
const React18 = window.React;
// components/core/Select.module.css
var Select_default = {
  wrapper: "Select_wrapper",
  label: "Select_label",
  selectWrap: "Select_selectWrap",
  select: "Select_select",
  placeholder: "Select_placeholder",
  error: "Select_error",
  disabled: "Select_disabled",
  chevron: "Select_chevron",
  errorMsg: "Select_errorMsg"
};

// components/core/Select.tsx
function Select({
  label,
  options = [],
  value,
  onChange,
  placeholder = "Select\u2026",
  disabled = false,
  error,
  id,
  className,
  style,
  ...rest
}) {
  const field = useFormField();
  const selectId = id || field?.id || (label ? `select-${String(label).toLowerCase().replace(/\s+/g, "-")}` : void 0);
  const errorId = selectId && error ? `${selectId}-error` : void 0;
  const describedBy = [errorId, field?.errorId].filter(Boolean).join(" ") || void 0;
  const hasValue = Boolean(value);
  return /* @__PURE__ */ React18.createElement("div", { className: cn(Select_default.wrapper, className), style }, label && /* @__PURE__ */ React18.createElement("label", { htmlFor: selectId, className: Select_default.label }, label), /* @__PURE__ */ React18.createElement("div", { className: Select_default.selectWrap }, /* @__PURE__ */ React18.createElement(
    "select",
    {
      id: selectId,
      value: value ?? "",
      onChange: (e) => onChange?.(e.target.value),
      disabled,
      required: field?.required || void 0,
      "aria-invalid": Boolean(error) || Boolean(field?.invalid),
      "aria-describedby": describedBy,
      className: cn(
        Select_default.select,
        error && Select_default.error,
        !hasValue && Select_default.placeholder,
        disabled && Select_default.disabled
      ),
      ...rest
    },
    placeholder && /* @__PURE__ */ React18.createElement("option", { value: "", disabled: true, hidden: true }, placeholder),
    options.map((opt) => /* @__PURE__ */ React18.createElement("option", { key: opt.value, value: opt.value }, opt.label))
  ), /* @__PURE__ */ React18.createElement(
    "svg",
    {
      "aria-hidden": "true",
      width: "12",
      height: "7",
      viewBox: "0 0 12 7",
      fill: "none",
      className: Select_default.chevron
    },
    /* @__PURE__ */ React18.createElement("path", { d: "M1 1l5 5 5-5", stroke: "currentColor", strokeWidth: "1.6", strokeLinecap: "round" })
  )), error && /* @__PURE__ */ React18.createElement("span", { id: errorId, className: Select_default.errorMsg }, error));
}

// components/core/SocialLink.tsx
const React19 = window.React;
// components/core/SocialLink.module.css
var SocialLink_default = {
  link: "SocialLink_link",
  icon: "SocialLink_icon"
};

// components/core/SocialLink.tsx
var ICONS = {
  linkedin: "assets/icon-linkedin.svg",
  instagram: "assets/icon-instagram.svg",
  youtube: "assets/icon-youtube.svg"
};
function SocialLink({ network, label, href = "#", base = "", style, className, ...rest }) {
  const src = base + (ICONS[network] || ICONS.linkedin);
  return /* @__PURE__ */ React19.createElement(
    "a",
    {
      href,
      className: cn(SocialLink_default.link, className),
      style,
      ...rest
    },
    /* @__PURE__ */ React19.createElement(
      "img",
      {
        src,
        alt: "",
        "aria-hidden": "true",
        className: SocialLink_default.icon
      }
    ),
    label || network
  );
}

// components/core/Stat.tsx
const React20 = window.React;
// components/core/Stat.module.css
var Stat_default = {
  root: "Stat_root",
  value: "Stat_value",
  xl: "Stat_xl",
  lg: "Stat_lg",
  md: "Stat_md",
  label: "Stat_label",
  labelXl: "Stat_labelXl"
};

// components/core/Stat.tsx
function Stat({ value, label, size = "lg", className, style, ...rest }) {
  return /* @__PURE__ */ React20.createElement("div", { className: cn(Stat_default.root, className), style, ...rest }, /* @__PURE__ */ React20.createElement("span", { className: cn(Stat_default.value, Stat_default[size]) }, value), label && /* @__PURE__ */ React20.createElement("span", { className: cn(Stat_default.label, size === "xl" && Stat_default.labelXl) }, label));
}

// components/core/Tabs.tsx
const React21 = window.React;
// components/core/Tabs.module.css
var Tabs_default = {
  root: "Tabs_root",
  tablist: "Tabs_tablist",
  tab: "Tabs_tab",
  active: "Tabs_active",
  panel: "Tabs_panel"
};

// components/core/Tabs.tsx
function Tabs({ tabs = [], defaultIndex = 0, selectedIndex, onTabChange, className, style, ...rest }) {
  const [internalActive, setInternalActive] = React21.useState(defaultIndex);
  const uid = React21.useId();
  const tabRefs = React21.useRef([]);
  const isControlled = selectedIndex !== void 0;
  const active = isControlled ? selectedIndex : internalActive;
  const setActive = (i) => {
    if (!isControlled) setInternalActive(i);
    onTabChange?.(i);
  };
  const onKeyDown = (e, i) => {
    if (e.key === "ArrowRight") {
      const next = (i + 1) % tabs.length;
      setActive(next);
      tabRefs.current[next]?.focus();
    } else if (e.key === "ArrowLeft") {
      const prev = (i - 1 + tabs.length) % tabs.length;
      setActive(prev);
      tabRefs.current[prev]?.focus();
    }
  };
  return /* @__PURE__ */ React21.createElement("div", { className, style, ...rest }, /* @__PURE__ */ React21.createElement("div", { role: "tablist", className: Tabs_default.tablist }, tabs.map((tab, i) => /* @__PURE__ */ React21.createElement(
    "button",
    {
      key: i,
      ref: (el) => {
        tabRefs.current[i] = el;
      },
      role: "tab",
      "aria-selected": active === i,
      "aria-controls": `tabpanel-${uid}-${i}`,
      id: `tab-btn-${uid}-${i}`,
      tabIndex: active === i ? 0 : -1,
      onClick: () => setActive(i),
      onKeyDown: (e) => onKeyDown(e, i),
      className: cn(Tabs_default.tab, active === i && Tabs_default.active)
    },
    tab.label
  ))), tabs.map((tab, i) => /* @__PURE__ */ React21.createElement(
    "div",
    {
      key: i,
      id: `tabpanel-${uid}-${i}`,
      role: "tabpanel",
      "aria-labelledby": `tab-btn-${uid}-${i}`,
      className: cn(Tabs_default.panel, active === i && Tabs_default.active)
    },
    tab.content
  )));
}

// components/core/Textarea.tsx
const React22 = window.React;
// components/core/Textarea.module.css
var Textarea_default = {
  wrapper: "Textarea_wrapper",
  label: "Textarea_label",
  textarea: "Textarea_textarea",
  error: "Textarea_error",
  disabled: "Textarea_disabled",
  errorMsg: "Textarea_errorMsg"
};

// components/core/Textarea.tsx
function Textarea({
  label,
  value,
  onChange,
  placeholder,
  rows = 4,
  disabled = false,
  error,
  id,
  className,
  style,
  ...rest
}) {
  const field = useFormField();
  const taId = id || field?.id || (label ? `textarea-${String(label).toLowerCase().replace(/\s+/g, "-")}` : void 0);
  const errorId = taId && error ? `${taId}-error` : void 0;
  const describedBy = [errorId, field?.errorId].filter(Boolean).join(" ") || void 0;
  return /* @__PURE__ */ React22.createElement("div", { className: cn(Textarea_default.wrapper, className), style }, label && /* @__PURE__ */ React22.createElement("label", { htmlFor: taId, className: Textarea_default.label }, label), /* @__PURE__ */ React22.createElement(
    "textarea",
    {
      id: taId,
      value,
      onChange: (e) => onChange?.(e.target.value),
      placeholder,
      rows,
      disabled,
      required: field?.required || void 0,
      "aria-invalid": Boolean(error) || Boolean(field?.invalid),
      "aria-describedby": describedBy,
      className: cn(
        Textarea_default.textarea,
        error && Textarea_default.error,
        disabled && Textarea_default.disabled
      ),
      ...rest
    }
  ), error && /* @__PURE__ */ React22.createElement("span", { id: errorId, className: Textarea_default.errorMsg }, error));
}

// components/core/Toast.tsx
const React23 = window.React;
// components/core/Toast.module.css
var Toast_default = {
  toast: "Toast_toast",
  visible: "Toast_visible",
  urgent: "Toast_urgent",
  message: "Toast_message",
  dismissBtn: "Toast_dismissBtn"
};

// components/core/Toast.tsx
function Toast({ message, type = "info", duration = 4e3, onDismiss, className, style, ...rest }) {
  const [visible, setVisible] = React23.useState(false);
  const onDismissRef = React23.useRef(onDismiss);
  React23.useEffect(() => {
    onDismissRef.current = onDismiss;
  });
  React23.useEffect(() => {
    const enter = setTimeout(() => setVisible(true), 16);
    if (duration > 0) {
      const dismiss = setTimeout(() => {
        setVisible(false);
        setTimeout(() => onDismissRef.current?.(), 300);
      }, duration);
      return () => {
        clearTimeout(enter);
        clearTimeout(dismiss);
      };
    }
    return () => clearTimeout(enter);
  }, [duration]);
  const isUrgent = type === "error" || type === "warning";
  return /* @__PURE__ */ React23.createElement(
    "div",
    {
      ...rest,
      role: isUrgent ? "alert" : "status",
      "aria-live": isUrgent ? "assertive" : "polite",
      "aria-atomic": "true",
      className: cn(Toast_default.toast, visible && Toast_default.visible, isUrgent && Toast_default.urgent, className),
      style
    },
    /* @__PURE__ */ React23.createElement("p", { className: Toast_default.message }, message),
    /* @__PURE__ */ React23.createElement(
      "button",
      {
        onClick: () => {
          setVisible(false);
          setTimeout(() => onDismissRef.current?.(), 300);
        },
        "aria-label": "Dismiss notification",
        className: Toast_default.dismissBtn
      },
      "\xD7"
    )
  );
}


  __ds_ns.Accordion = typeof Accordion !== 'undefined' ? Accordion : undefined;
  __ds_ns.ArrowLink = typeof ArrowLink !== 'undefined' ? ArrowLink : undefined;
  __ds_ns.Badge = typeof Badge !== 'undefined' ? Badge : undefined;
  __ds_ns.Breadcrumb = typeof Breadcrumb !== 'undefined' ? Breadcrumb : undefined;
  __ds_ns.Button = typeof Button !== 'undefined' ? Button : undefined;
  __ds_ns.Checkbox = typeof Checkbox !== 'undefined' ? Checkbox : undefined;
  __ds_ns.CheckboxGroup = typeof CheckboxGroup !== 'undefined' ? CheckboxGroup : undefined;
  __ds_ns.FormField = typeof FormField !== 'undefined' ? FormField : undefined;
  __ds_ns.Input = typeof Input !== 'undefined' ? Input : undefined;
  __ds_ns.Logo = typeof Logo !== 'undefined' ? Logo : undefined;
  __ds_ns.MediaCard = typeof MediaCard !== 'undefined' ? MediaCard : undefined;
  __ds_ns.Modal = typeof Modal !== 'undefined' ? Modal : undefined;
  __ds_ns.NavigationBar = typeof NavigationBar !== 'undefined' ? NavigationBar : undefined;
  __ds_ns.Radio = typeof Radio !== 'undefined' ? Radio : undefined;
  __ds_ns.RadioGroup = typeof RadioGroup !== 'undefined' ? RadioGroup : undefined;
  __ds_ns.SectionHeader = typeof SectionHeader !== 'undefined' ? SectionHeader : undefined;
  __ds_ns.Select = typeof Select !== 'undefined' ? Select : undefined;
  __ds_ns.SocialLink = typeof SocialLink !== 'undefined' ? SocialLink : undefined;
  __ds_ns.Stat = typeof Stat !== 'undefined' ? Stat : undefined;
  __ds_ns.Tabs = typeof Tabs !== 'undefined' ? Tabs : undefined;
  __ds_ns.Tag = typeof Tag !== 'undefined' ? Tag : undefined;
  __ds_ns.Textarea = typeof Textarea !== 'undefined' ? Textarea : undefined;
  __ds_ns.Toast = typeof Toast !== 'undefined' ? Toast : undefined;
})();